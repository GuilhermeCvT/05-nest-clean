...
import { InMemoryQuestionAttachmentsRepository } from './in-memory-question-attachments-repositories'
...
export class InMemoryQuestionsRepository implements QuestionsRepository {
  ...
  async findDetailsBySlug(slug: string) {
    ...
    const attachments = questionAttachments.map(questionAttachment => {
      ...
      if(!attachment)
        throw new Error(`Author with ID "${questionAttachment.attachmentId.toString()}" does not exist`)
